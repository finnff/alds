...
class coordinationGraph:
    ...
    def __init__(self, noNodes, pEdge, noActions, seed=42):
        ...
        """
        random.seed(seed)
        # for each node, a list of nodes it is connected to
        self.nodesAndConnections = dict()
        # A dictionary of tuples (of two decision variables) to an object of the edge class
        self.edges = dict()
        # First make sure that the entire graph is connected (connecting all nodes to the next one)
        for i in range(noNodes):
            if i == 0:
                ...
                eddy = edge(i, i + 1, noActions, noActions)
                self.edges[(i, i + 1)] = eddy
        tuplist = [(x, y) for x in range(noNodes)
                   for y in range(x + 2, noNodes)]
    ...
    def evaluateSolution(self, solution):
        ...
        for i in range(len(solution)):
            for j in self.nodesAndConnections[i]:
                if(j > i):
                    print("("+str(i)+","+str(j)+") -> " +
                          str(self.edges[(i, j)].localReward(solution[i], solution[j])))
                    result += self.edges[(i, j)
                                         ].localReward(solution[i], solution[j])
        return result
